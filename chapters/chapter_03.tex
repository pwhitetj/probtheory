...
% \end{solution}
\begin{example}
Write an expression for $\Pr[a<X\leq b, c< Y \leq d$] in terms of $F$. %Be careful to consider all the cases!
\end{example}
\begin{solution}
...
\end{example}
\begin{solution}
%% Solution 1: Taylor, with some approximatinos
...
\end{example}
\begin{solution}
% $f_X(x) = 1, f_{X|Y}(X|Y) = \frac 1 x$, so then the joint pdf $f_{XY}(XY) = \frac 1x$. To find $\Pr[Y\geq \frac 12]$, we integrate, either with $$\int_{\frac 12}^1 \int_{1-y}^1 \frac 1x dx dy$$, or $1 - \int_{\frac 12}^1 \int_{1-x}^{\frac 12} \frac 1x \dy \dx$. $\frac 12 (1 + \ln 2)$
\end{solution}
\begin{example}
...
% change to polar, do the jacobian, leaving the k in -> 5k
% k = 1/16, so the answer is 5/16. 
% issue at 0?
\begin{example}
